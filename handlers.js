    ...
    async showAtlanticQRIS(ctx, amount, depositId, atlanticData) {
        try {
            ...
            console.log('📊 Atlantic data:', {
                id: atlanticData.id,
                hasQRImage: !!atlanticData.qr_image,
                hasQRString: !!atlanticData.qr_string,
                qrImageUrl: atlanticData.qr_image
            });
            ...
            // Coba kirim QR Image yang digenerate dari QR string
            if (atlanticData.qr_string) {
                try {
                    ...
                    // Fallback ke QR string
                    await this.sendQRStringFallback(ctx, amount, depositId, atlanticData, buttons);
                }
            } else if (atlanticData.qr_image && atlanticData.qr_image.includes('atlantich2h.com')) {
                try {
                    console.log('📤 Attempting to send QR image from URL:', atlanticData.qr_image);
                    await ctx.replyWithPhoto(atlanticData.qr_image, {
                        caption: `💳 QRIS PAYMENT\n\n` +
                               `💰 ${this.formatRp(amount)}\n` +
                               `⏰ ${expiredTime}\n` +
                               `🆔 ${depositId.substring(0, 10)}...\n` +
            ...
            // Kirim instruksi
            await this.sendAdditionalInstructions(ctx, amount, depositId);

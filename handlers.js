        ...
        try {
            ...
            if (scripts.length === 0) {
                await ctx.reply(
                    ...
                    'Tambahkan script terlebih dahulu.',
                    {
                        reply_markup: {
                            inline_keyboard: [
                                [{ text: '➕ Tambah Script', callback_data: 'admin_add_script' }],
            ...
            scripts.slice(0, 10).forEach((script, index) => {
                text += 
                    `${index + 1}. *${script.name}*\n` +
                    `   🆔 ID: \`${script.id}\`\n` +
                    `   💰 ${this.formatRp(script.price)}\n` +
                    `   📦 Stok: ${script.stock} unit\n` +
                    `   📊 Status: ${script.status === 'available' ? '✅ TERSEDIA' : '❌ HABIS'}\n\n`;
            });
            
            text += 
                `*📝 CARA MENAMBAH STOK:*\n` +
                `1. Salin ID script (contoh: S1767015132679)\n` +
                `2. Klik tombol "📈 Tambah Stok" di Admin Panel\n` +
                `3. Tempel ID script\n` +
                `4. Masukkan jumlah stok\n\n` +
                `*⚠️ CATATAN:*\n` +
                `• Minimal 1 stok, maksimal 1000\n` +
                `• Stok akan ditambahkan ke stok saat ini\n` +
                `• Script otomatis aktif jika stok > 0`;
            ...
            await ctx.reply(
                text,
                {
                    ...
                    reply_markup: {
                        inline_keyboard: [
                            [{ text: '📈 Tambah Stok Script', callback_data: 'admin_add_stock' }],

...
const qrisMessages = new Map(); // Menyimpan pesan QRIS untuk dihapus nanti
...
/**
 * Fungsi untuk menghapus pesan QRIS
 * @param {number} userId - ID user
 * @param {string} depositId - ID deposit
 */
async function deleteQRISMessages(userId, depositId) {
    try {
        ...
        for (const msg of depositMessages) {
            try {
                ...
                console.log(`🗑️ Deleted QRIS message for deposit ${depositId}`);
            } catch (deleteError) {
                console.error('Error deleting QRIS message:', deleteError.message);
            }
        }
...
/**
 * Fungsi untuk menyimpan info pesan QRIS
 * @param {number} userId - ID user
 * @param {string} depositId - ID deposit
    ...
    try {
        ...
        if (updateResult.success) {
            if (updateResult.status === 'approved') {
                // Hapus pesan QRIS
                await deleteQRISMessages(ctx.from.id, depositId);
...
// ==================== CLEANUP ROUTINE ====================
// Bersihkan cache QRIS messages yang sudah expired (lebih dari 24 jam)
setInterval(() => {
    const now = Date.now();

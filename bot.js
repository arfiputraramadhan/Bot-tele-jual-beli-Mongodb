...
const activeAutoChecks = new Map(); // Store active auto-check intervals
    ...
    setInterval(async () => {
        try {
            ...
            for (const deposit of pendingDeposits) {
                // Skip jika sudah melebihi max tries
                ...
                try {
                    ...
                    // Gunakan instant check untuk auto-check
                        ...
                        if (updateResult.success && updateResult.status === 'approved') {
                            ...
                            try {
                                await bot.telegram.sendMessage(
                                    ...
                                    `🎉 Saldo telah ditambahkan: ${handlers.formatRp(deposit.amount)}\n` +
...
async function startManualCheckForDeposit(userId, depositId) {
    try {
        ...
        // Simpan interval ID untuk bisa di-cancel nanti
        const checkInterval = setInterval(async () => {
            try {
                ...
                // Gunakan instant check dengan action=true untuk pencairan
        ...
        }, 3000); // Check every 3 seconds for manual checks
    ...
    try {
        ...
        if (state?.action === 'DEPOSIT_AMOUNT_ATLANTIC' && state.step === 'amount') {
            ...
            try {
                ...
                // Mulai auto-check untuk deposit ini
                    ...
                    try {
                        ...
                        if (result.success) {
                            ...
                            await ctx.reply(
                                ...
                                `💰 Harga: ${handlers.formatRp(result.script.price)}\n\n` +
